...
4. **same-Trip rule** — for two directly consecutive **selected** rail legs that share a `TripID`, their Trip snapshots must first be **structurally identical**, compared field by field (`stopSequence`, `lineSegments`, `coverage`, `serviceTypeSegments`) — never through `Trip`'s ID-only `==` — because indices from different snapshots are not comparable; differing snapshots make the Journey **incompatible** and it is rejected, never reconciled. With identical snapshots, the next leg must board at a **higher** index than the previous alights: an **equal** index is one uninterrupted ride cut in two, a fake transfer (DEC-009, Rule 16), and a **lower** index is unsupported backward traversal. A later recurrence of the same `TripID` is never inferred — representing one needs a service-date or execution identity and a later decision. The same `TripID` separated by another leg is valid (these checks apply only to directly consecutive legs);
