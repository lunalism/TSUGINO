...
4. **same-Trip rule** — for two directly consecutive **selected** rail legs that share a `TripID`, their Trip snapshots must first be **structurally identical**, compared field by field (`stopSequence`, `lineSegments`, `coverage`, `serviceTypeSegments`) — never through `Trip`'s ID-only `==` — because indices from different snapshots are not comparable; differing snapshots make the Journey **incompatible** and it is rejected, never reconciled. With identical snapshots, the pair is rejected when the next boards at the index where the previous alights: one uninterrupted ride cut in two, a fake transfer (DEC-009, Rule 16). Nothing else is rejected on this ground: the same `TripID` separated by another leg is valid (the compatibility check applies only to directly consecutive legs), and so is a later-index reboard with identical snapshots; reboarding at an earlier index needs an operating date and is a Phase 5 check;
