...
The Xcode bootstrap (Track A of `PHASE_0_SCOPE_LOCK.md`) does not depend on the open items. Phase 0 *exit* requires the remaining A3 work; the RK-2 product decision is closed (DEC-047) and the **A4 cross-operator canonical identity work is completed — PASS WITH AMBIGUITIES, accepted as DEC-048** (§6.5), leaving the Shinjuku authoritative-evidence review as its only open item (RK-18).
