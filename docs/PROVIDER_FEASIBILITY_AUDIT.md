...
| A4 | Sample station names for Toei / Tokyo Metro / Basic-License operators; measure JP/EN/KO coverage. **Progress (2026-09-18, B4):** Toei done — JA/EN 141/141 station names, 6/6 route names, 54/54 headsigns; Korean 0 (§6.1.3). **Progress (2026-09-18, B6B/B6C):** Tokyo Metro done — static JA/EN 144/144 station names, 9/9 route names, 93/93 headsigns, Korean 0; `odpt:Railway` Korean titles 9/10 and station-order titles observed (provider input, review required) (§6.2.5); 27 exact JA+EN cross-operator name-match candidates with Toei plus one orthographic variant (市ヶ谷/市ケ谷) — candidates only. **Remaining:** cross-operator canonical station identity for the Toei + Tokyo Metro launch set using station codes/topology, not names alone — **launch-relevant under DEC-047 and the selected next Phase 0 task** (offline, retained payloads only); other Basic-License operators (deferred, DS-08); reviewed project-owned Korean dataset and aliases | RK-9, RK-14 | coverage table; canonical identity proposal |
...
The Xcode bootstrap (Track A of `PHASE_0_SCOPE_LOCK.md`) does not depend on the open items. Phase 0 *exit* requires the remaining A3 work and the A4 cross-operator canonical identity work; the RK-2 product decision is closed (DEC-047).
