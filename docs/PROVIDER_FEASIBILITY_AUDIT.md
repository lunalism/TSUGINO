...
### 3.12 Written-confirmation backlog (directed to the ODPT secretariat; operators only if ODPT refers the question) — nothing has been sent

| # | Priority | Question |
|---|---|---|
| 1 | high | Do App Store screenshots/previews containing provider data count as permitted display **within the Deliverable** (S2 Art. 4(2)(3))? |
| 2 | high | May official Tokyo Metro station-number marks / line symbols be used on marketing surfaces? |
| 3 | high | Is system scaling, tinting, monochrome, or animated presentation acceptable for the marks in Live Activity, Dynamic Island, or widgets (PDF §3, §4, §8(2))? |
| 4 | high | May official marks be shipped inside the app binary given the redistribution (Art. 8(4)) and deletion (Art. 13(3)) clauses? |
| 5 | high | May normalized static GTFS be bundled offline in the binary and updated after release (Art. 8(4), 13(3); S6 §2.2)? |
| 6 | high | Does non-restorable project canonical data fall outside the Art. 8(4) redistribution prohibition? |
| 7 | medium | May provider strings and dynamic status/Alert text be translated to Korean (human or machine), and how must machine-translation disclosure (S6 §5) appear? |
| 8 | medium | May exact official line colours be used as standalone UI tokens (colour-only trademark/design status)? |
| 9 | medium | How does termination deletion (Art. 13(3)) apply to caches on already-installed user devices? |
| 10 | low | Preferred Toei CC BY attribution placement (in-app screen vs. App Store description) |
