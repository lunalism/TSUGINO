...
The same per-service evaluation and the same production-eligibility gate apply to every future operator added under the staged Tokyo urban-rail and Airport Rail expansion (DEC-058): no operator or line is treated as supported because of its name, its catalog presence, or its product priority. A route-search result may include an unsupported line or segment as untrackable presentation; that never makes it a supported line.
