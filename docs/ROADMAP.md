...
**Status: S5a contract accepted (DEC-062, 2026-09-23); implementation not started. S5b not started; its decision identifier is unassigned.**

**S5 is subdivided (DEC-062):**

- **S5a — Journey structure.** `Journey { id, legs }` with `JourneyID`-only identity and position-addressed legs; `JourneyLeg` = `rail(RailLeg)` | `walkingTransfer(WalkingTransfer)`; `RailLeg` = `unselected(RailLegAnchors)` (boarding and alighting stations only — no Trip, index, time, or route) | `selected(SelectedRailTrip)` (a `Trip` snapshot with boarding and alighting **indices**, anchors derived); a selection must preserve an unselected leg's anchors (`RailLegAnchors.admits`), and performing the binding is Phase 5. Journey invariants: non-empty; rail first and last; station **continuity** between consecutive legs using local values only; no consecutive walking legs; a leg's two ends are distinct stations; and the **same-Trip rule** — directly consecutive selected legs with the same `TripID` must carry structurally identical snapshots (compared field by field, never by `Trip`'s ID-only equality; differing snapshots are rejected as incompatible, not reconciled), and with identical snapshots the next leg must board at a higher index than the previous alights — an equal index is rejected as a fake transfer and a lower index as unsupported backward traversal; a later recurrence of the same `TripID` is never inferred and needs a later decision introducing service-date or execution identity. An all-unselected Journey is structurally valid. A walking transfer is **stated, not verified**: pedestrian-connection verification is Phase 2, and walking time and guidance are Phase 10. Legs are not `Equatable`. Both enums encode as one keyed `kind` discriminator plus one payload key named after the case, rejecting unknown, missing, contradictory, and invalid payloads.
- **S5b — Journey runtime state:** `JourneyState`, `JourneyPhase`, `JourneyEvent`, realtime freshness (DEC-024), interruption reasons (DEC-031), typed errors (`ARCHITECTURE.md` §34), and tracking readiness — vocabulary only; behaviour stays Phase 5 (DEC-050). **Not started; its decision identifier is unassigned.**

**S5a tests** must cover: `JourneyID`-only identity with explicit comparison of legs; anchor and walking-endpoint distinctness; selected-index bounds, ordering, and distinct end stations; boarding at a repeated station by index; `admits` accepting anchor-preserving and rejecting anchor-changing selections; continuity across every combination of unselected, selected, and walking legs; rail-first and rail-last; consecutive walking legs; consecutive same-`TripID` legs with identical snapshots joined at an equal index (rejected as a split), a lower index (rejected as backward traversal), and a higher index (accepted), including repeated-station continuity on a loop-plus-tail snapshot, through construction and decoding; separated same-Trip legs accepted; same-`TripID` consecutive legs whose snapshots differ by a stop inserted or removed before the join, or only in `lineSegments`, `coverage`, or `serviceTypeSegments`, rejected as incompatible; a multi-line through service as one leg; an all-unselected Journey; and `Codable` round-trips plus rejection of missing, unknown, contradictory, and invalid payloads, extreme decoded indices, and every Journey invariant.
