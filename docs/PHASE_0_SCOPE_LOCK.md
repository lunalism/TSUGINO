...
- **Next non-blocked Phase 0 work while awaiting ODPT (determined 2026-09-19; no product implementation):** (1) **RK-2 launch-scope product decision** — human decision on a Toei-trip-level + Metro/Liner-degraded launch scope; not dependent on the reply. (2) **A4 cross-operator canonical station identity** — offline analysis of the already-retained Toei and Tokyo Metro static payloads (station codes/topology; the 27 exact JA+EN match candidates and the 市ヶ谷/市ケ谷 variant), no network or credential. (3) **Attribution/notice copy** — draft the Legal/Data Sources screen text for the Toei CC BY attribution (with modification indication) and the Tokyo Metro three-part notice as documentation; both obligations are already verified and were not part of the inquiry. (4) **RK-11 decoder approach** — research note on a dependency-free GTFS-RT decoding design for Phase 4 (no code). (5) **Conservative-path design notes** — provider-cache deletion capability and non-restorable canonical data can be designed now under the current restrictions; only their *relaxation* waits for ODPT. (6) **Opportunistic evidence, only with explicit user authorization for API access** — Liner Alert/status observation and Tokyo Metro disruption-state sampling when a disruption naturally occurs; longer-window Toei sampling if the gate requires it. **Blocked on the ODPT reply (do not start):** offline binary bundling decisions, Tokyo Metro Korean string/incident translation, official-colour tokens as standalone marks, any screenshot/marketing plan containing Basic-License data, and any official-mark use. Track A's remaining item (physical Dynamic Island validation) is unrelated to ODPT and waits for capable hardware and user authorization.
