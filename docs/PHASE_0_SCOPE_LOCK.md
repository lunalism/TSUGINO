...
- **Selected next Phase 0 task (exactly one, 2026-09-19): A4 — cross-operator canonical station identity for the Toei + Tokyo Metro launch set.** Offline analysis of the already-retained Toei and Tokyo Metro static payloads and `odpt:Railway` station orders, using station codes and line topology rather than names alone (the 27 exact JA+EN name-match candidates and the 市ヶ谷/市ケ谷 variant are inputs, not conclusions); no network, credential, code, or repository data artifact. **Completion criterion:** a documented canonical station identity proposal covering all 141 Toei and 144 Tokyo Metro canonical names — every interchange resolved to one canonical station or explicitly listed as ambiguous with the reason — recorded in the audit (§6/§9 DS-15, RK-14) with aggregate counts only. Rationale: with both subway operators in scope, wrong interchange identity is the highest remaining launch risk to route results, transfers, and search; it needs no ODPT response and no implementation. Other candidates (attribution copy, RK-11 decoder note, conservative cache-deletion design) remain queued behind it. **Blocked on the ODPT reply (do not start):** offline binary bundling decisions, Tokyo Metro Korean string/incident translation, official-colour tokens as standalone marks, any screenshot/marketing plan containing Basic-License data, and any official-mark use. Track A's remaining item (physical Dynamic Island validation) is unrelated to ODPT and waits for capable hardware and user authorization.
