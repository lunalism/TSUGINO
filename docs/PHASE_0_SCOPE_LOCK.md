...
- **B10 — public catalog re-check and Toei GTFS-Pathways acquisition/offline analysis: completed (2026-09-20, credential-free; audit §2.3, §3.5.4, §6.6).** Unauthenticated public re-check: the developer-site page asset is byte-identical to the 2026-09-18 retrieval, so S1–S6 terms are unchanged; **Tokyo Metro GTFS-RT TripUpdate and VehiclePosition are still not cataloged** (ten datasets, realtime dataset exposes only the GTFS-RT Alert resource); **no public numeric request-rate limit exists** — the official FAQ states response-size / query-shape constraints only (1,000 records for ordinary static data, 10,000 for `odpt:BusstopPole`, dynamic responses not subject to that cap, OR search limited to 10 values), which are **not** rate limits. The Toei **GTFS-Pathways** resource is still published, still labelled 【コンテスト期間限定公開】, with **no end date and no catalog timestamps**, and a credential-free URL; **one snapshot was acquired and preserved in owner-only external research evidence storage outside the repository** (CC BY 4.0; 828,896 bytes; SHA-256 `8003800473770f40eb0a2eabeef1e309c052100aab85c5376b831eec089d95f4`; ZIP and CRC validation passed). No credential, token, authenticated endpoint, or Keychain access was used; no signed redirect URL was printed or persisted; no archive or extracted record entered the repository. **Offline analysis — `ACQUIRED_AND_ANALYZED_NO_CROSS_OPERATOR_EVIDENCE`:** twelve Ōedo Line station structures only (2,741 stops / 2,870 pathways / 18 levels, no `transfers.txt`), Toei single-agency, **zero pathway edges between any two station structures**, and no Tokyo Metro identifier, 丸ノ内線 reference, `M08` code, cross-operator transfer, or shared cross-operator parent anywhere in it. **DEC-048 is unchanged**: Toei 新宿 and Tokyo Metro 新宿 remain separate operator-scoped identities, the relationship remains explicitly ambiguous, and no transfer edge is inferred — the dataset models no inter-station connectivity, so its silence proves neither sameness nor distinctness. The GTFS-Pathways evidence path is now exhausted for this question; the other authoritative paths stay open (RK-18). RK-17 is recharacterized from loss-before-acquisition to **snapshot currency and continued upstream availability**. A further finding for Phase 2: the Pathways archive changes station/child-stop shape (149 → 2,741 stops; 10,120 `stop_times` rows at those twelve stations reference 26 platform-level child stop ids), so adopting it as the Toei static source would be a controlled identity migration, not a drop-in substitution; the `stop_code` set is identical across both archives and is the stable join-key candidate (audit §6.6.3, §11).
...
Living documents touched by the 2026-09-20 B10 evidence update (aggregate-only; no provider payload):
